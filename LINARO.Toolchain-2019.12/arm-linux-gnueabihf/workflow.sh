...
TOOLCHAIN_INSTALL_DIR="${TOOLCHAIN_INSTALL_DIR:-LINARO.Toolchain-2019.12}"
TOOLCHAIN_MANIFEST="$(readlink -f "${TOOLCHAIN_MANIFEST:-gcc-linaro-arm-linux-gnueabihf-manifest.txt}")"
TOOLCHAIN_ABE_REVISION="${TOOLCHAIN_ABE_REVISION:-d2f9b66a9f00502eb56db3fcb8103522d9bc8afd}"
...
TOOLCHAIN_ARCHIVE="${TOOLCHAIN_ARCHIVE:-gcc-9.2-linaro-2019.12-arm-linux-gnueabihf.tar.gz}"
GIT="${GIT:-git}"
...
check() {
  ...
  sysroot="$(readlink -f "$builddir/sysroots/$TOOLCHAIN_TARGET_MARCH")"
  if [[ ! -d "$sysroot" ]]; then
